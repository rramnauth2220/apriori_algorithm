...
public class Apriori extends Observable {
    ...
    private void configure(String[] args) throws Exception {        
    	...
    	while (data_in.ready()) {    		
    		...
    		StringTokenizer t = new StringTokenizer(line," ");
                //System.out.println("row: " + tokenReader(t));
    		//createOrigin(tokenArray(t));
                //printArray(tokenArray(t));
    ...
    private void createOrigin(int[] in) {
        int[] out = new int[in.length];
        System.arraycopy(in, 0, out, 0, in.length);
        originsets.add(out);
        printArray(originsets.get(originsets.size()-1));
